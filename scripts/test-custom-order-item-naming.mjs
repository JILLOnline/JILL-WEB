...
assert.match(formatter, /clean_segment == 'Print'/, 'Custom Order labels must remove plain production-method segments');
assert.match(formatter, /clean_segment == 'Photo, Text & Design'/, 'Custom Order labels must remove customization marketing segments');
assert.match(formatter, /clean_segment contains 'Includes '/, 'Custom Order labels must remove included-accessory wording generically');
