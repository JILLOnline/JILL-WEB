...
| Product Options | one Product Options state/renderer owner when introduced | product capabilities, Shopify merchandise quantity, resolver-owned customization-unit multiplier/default, universal form engine | collection-specific option implementations, product-title/count inference, a second units-per-quantity rule |
